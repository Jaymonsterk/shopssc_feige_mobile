<template>
    <view class="container">
        ...
        <view class="nav_wrap">
            ...
            <view class="title">{{i18n.head_title}}</view>
        ...
        <view class="list_wrap">
        	<view class="list" v-for="(item,index) in list" :key='index'>
        		<view class="item">
        			<text class="new_type" v-if="item.typeTitle == '下单'">{{i18n.order}}</text>
        			<text class="new_type" v-else-if="item.typeTitle == '分售'">{{i18n.fen}}</text>
        			<text class="money" :class="Number(item.money)<0?'reduce':''"><text v-text="Number(item.money)>0?'+':''"></text>{{Math.floor(Number(item.money))}}</text>
        		</view>
        		
        		<view class="item">
        			<text class="time">{{item.timeCreated | format}}</text>
        			<text class="money_type">{{i18n.has}}：{{item.afterMoney}}</text>
        		</view>
        	</view>
	...
	export default {
		...
		methods: {
            ...
            getBilList(){
                ...
                this.$tools.Post("dock/member/member/moneyLog",params).then((res) =>{
                    if(res.errCode == 0){
                        //这里只会在接口是成功状态返回
