        ...
        <view class="contentWrap" @tap="isShowNewList = false">
            ...
            <view class="top_wrap">
				<view class="top_wrap_item2">
				    <view class="wrap_item2_left">
				        ...
				        <view class="item2_left_orderTxt">{{i18n.end}}</view>
				    </view>
                    ...
                    <view class="top_wrap_item1_right">
                        ...
                        <view class="item1_right_type" v-else>
                            <view class="item1_right_type_text1">
                                {{result.size == 0 ? i18n.tong : i18n.you}}
                            </view>
                            <view class="item1_right_type_text2">
                                {{result.quantity == 1 ? i18n.dan : i18n.shuang}}
                            </view>
                        </view>
            ...
            <view class="list_cont">
                <view class="list_cont_title">
                    <view class="cont_title_item" v-for="(item, index) of typeList" :key="index" :class="{active: arrLeng.includes(index)}" @tap="titleFun(index, item, item.name)">{{item.cnname}}</view>
                </view>
                <view class="list_cont_box">
<!--                    <view class="cont_box_item" :class="{activeItem: index == imgIndex }" v-for="(item, index) in brandImg" :key="index" @tap="brandImgFun(item,index)"><image :src="item.img" mode=""></image></view>
 -->
                      <view class="cont_box_item" :class="{activeItem: index == item.value }" v-for="(item, index) in brandImg" :key="index"><image :src="item.img" mode=""></image></view>
                ...
                <view class="footer_top" v-if="arrLeng.length > 0">
                    <view class="footer_top_title">
                        <view class="f_top_title_l">
                            {{i18n.dangqian}}：
                            <text class="balance">{{Math.floor(Number(allMoney))}}</text>
                        </view>
                        <view class="f_top_title_r" @tap="cleanUp">{{i18n.clear}}</view>
                    </view>
                    <view class="footer_top_cont">
                        <view class="f_top_cont_L">
                            <text class="reduce" @tap="reduceFun()">-</text>
                            <!-- <text class="numbers">{{multiple}}</text> -->
                ...
                <view class="footer_bottom">
					<view class="daboxk">
						<view class="footer_bottom_left">
						    {{i18n.gong}}{{arrLeng.length}}{{i18n.jian}}:{{i18n.heji}}<text class="b_money">{{total}}{{i18n.jifen}}</text>
						</view>
						<view class="footer_bottom_right" v-if="isShow">{{i18n.order_load}}</view>
						<view class="footer_bottom_right" v-else @tap="SubmitOrder()">{{i18n.submit}}</view>
					</view>
...
<script>
    ...
    export default {
        ...
        onLoad(options) {
			...
			this.getDeails(options.id)
            this.brandList = uni.getStorageSync('brandList');
            // this.initData() // 初始化
            this.getMoneyList() //产品金额列表
            if(options.Index || options.Index < 9){
              this.brandImg = [
                {img: require("@/static/order/"+options.Index+"/0.png"),value:-1,title:"李宁"},
                {img: require("@/static/order/"+options.Index+"/1.png"),value:-1,title:"361"},
                {img: require("@/static/order/"+options.Index+"/2.png"),value:-1,title:"鸿星尔克"},
                {img: require("@/static/order/"+options.Index+"/3.png"),value:-1,title:"特步"},
                {img: require("@/static/order/"+options.Index+"/4.png"),value:-1,title:"百雀羚"},
                {img: require("@/static/order/"+options.Index+"/5.png"),value:-1,title:"御泥坊"},
                {img: require("@/static/order/"+options.Index+"/6.png"),value:-1,title:"薇婼娜"},
                {img: require("@/static/order/"+options.Index+"/7.png"),value:-1,title:"相宜本草"},
                {img: require("@/static/order/"+options.Index+"/8.png"),value:-1,title:"华为"},
                {img: require("@/static/order/"+options.Index+"/9.png"),value:-1,title:"中兴"},
                {img: require("@/static/order/"+options.Index+"/10.png"),value:-1,title:"小米"},
                {img: require("@/static/order/"+options.Index+"/11.png"),value:-1,title:"vivo"},
                {img: require("@/static/order/"+options.Index+"/12.png"),value:-1,title:"国美"},
                {img: require("@/static/order/"+options.Index+"/13.png"),value:-1,title:"苏宁"},
                {img: require("@/static/order/"+options.Index+"/14.png"),value:-1,title:"美的"},
                {img: require("@/static/order/"+options.Index+"/15.png"),value:-1,title:"格力"},
              ]
            }else{
              this.brandImg = [
                {img: require("@/static/images/brand/pp_lining_1.png"),value:-1,title:"李宁"},
        ...
        methods: {
            getDeails(id){
				 this.$tools.Post("dock/product/product/detail",{id: id}).then((res) =>{
				 	uni.hideLoading()
				     if(res.errCode == 0){
				         console.log(res)
						 this.banraName = res.data.title;
						 this.branCode = res.data.name
						 this.objData.name = res.data.name
						 this.timeExpect = res.data.timeEx
						 this.timeInterval = res.data.timeIn
						 this.initData() 
				     } else {
				         uni.showToast({
				             title: res.errMsg,
            ...
            SubmitOrder(){
                ...
                uni.showModal({
                    title: '',
                    content: this.i18n.prop_title,
					cancelText: this.lang.cancel, // 取消按钮的文字  
					confirmText: this.lang.confimText, // 确认按钮的文字  
        ...
        computed: {
            ...
            // isShow () {
            //     return Number(this.remainTime) <= Number(self.timeInterval) ? true : false
            // }
			i18n () {
...
<style scoped lang="less">
    ...
    .business_listWrap{
                    ...
                    .index-header {
                        ...
                        .headerLeftMenu{
                            ...
                            .backIcon{
                                ...
                                width: 60rpx;
                                border-radius: 50%;
                                background: url('../../static/images/back_black.png') no-repeat center center;
        ...
        .top_wrap{
			height:408rpx;
			background: url(../../static/images/shangbbj.png) no-repeat center center;
                ...
                .top_wrap_item1_right{
                    ...
                    .item1_right_type{
                        ...
                        .item1_right_type_text1{
                            background: url(../../static/images/jisnk.png) no-repeat center center;
                            background-size: 100% 100%;
                            border-radius: 6rpx;
                            font-size: 32rpx;
                            color: #FF1E00;
                        ...
                        .item1_right_type_text2{
                            background: url(../../static/images/danjk.png) no-repeat center center;
                            background-size: 100% 100%;
                            border-radius: 6rpx;
                            font-size: 32rpx;
							width:193rpx;
            ...
            .top_wrap_item2{
                ...
                .wrap_item2_left{
                    ...
                    .item2_left_order{
						...
						height:64rpx;
						color:#fff;
						background: url(../../static/images/ds.png) no-repeat center;
						background-size: 100%;
						margin:0 auto;
                    }
                    .item2_left_orderTxt{
                ...
                .wrap_item2_right{
                    ...
                    .time{
                        width: 142rpx;
                        height: 112rpx;
                        background: url(../../static/images/hsk.png) no-repeat center;
                        background-size: 100%;
                        font-weight: 500;
                        line-height: 112rpx;
                        color: #FDF2DF;
            ...
            .list_cont_box{
                ...
                .cont_box_item{
                    ...
                    &.activeItem{
						...
						background: url(../../static/images/xuanz.png) no-repeat center;
						background-size: 100%;
                    }
                    &:nth-child(4n){
                        margin-right: 0rpx;
        ...
        .footer{
                ...
                .footer_top_cont{
                    ...
                    .f_top_cont_L{
                        ...
                        .plus{
                            ...
                            color: #FFFFFF;
                            line-height: 60rpx;
                            background: linear-gradient(45deg, #D137FB, #54129D);
                    ...
                    .f_top_cont_R{
						...
						background: linear-gradient(90deg, #D137FB, #54129D);
                        align-items: center;
                        background: #FFFFFF;
            ...
            .footer_bottom{
                background: url(../../static/images/huadqt.png) no-repeat center center;
                background-size: 100% 100%;
                display: flex;
                align-items: center;
				height:128rpx;
				.daboxk{
					...
					height: 88rpx;
					margin:0 auto;
					background: linear-gradient(0deg, #E63749, #EB5130);
					border-radius: 44rpx;
					display: flex;
					justify-content: space-between;
				}
                ...
                .footer_bottom_right{
                    width:231rpx;
					height:99rpx;
					background: url(../../static/images/xdkuang.png) no-repeat center center;
					background-size: 100% 100%;
                    font-size: 32rpx;
                    text-align: center;
					line-height:99rpx;
